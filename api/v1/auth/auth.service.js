   ...
   login(req, res, next) {
        ...
        user.save(function(err){
           ...
           console.log(user,"user");
            var token = _this.signToken({ _id: user._id, email: user.email, role:user.role ,  userName:user.firstName});
            return ResponseManager.respondWithSuccess(res, 200, "", {access_token: token, email:user.email , userName:user.firstName});
